...
static esp_err_t index_html_handler(httpd_req_t *req)
{
    ...
    const char *index_html =
        ...
        "#sparkline-timestamp-range { font-size: 0.8em; color: #aaa; text-align: center; margin-bottom: 10px; display: none; }"
        ...
        "li.active-preview { background-color: #4a4a4a; }"
        ...
        "<div id=\"sparkline-timestamp-range\"></div>"
        "<div id=\"sparkline-container\">"
        "  <div class=\"sparkline-item\"><h4>Temperature</h4><canvas id=\"spark-temp\" width=\"200\" height=\"40\"></canvas></div>"
        "  <div class=\"sparkline-item\"><h4>Pressure</h4><canvas id=\"spark-press\" width=\"200\" height=\"40\"></canvas></div>"
        "  <div class=\"sparkline-item\"><h4>Diff. Pressure</h4><canvas id=\"spark-diff-press\" width=\"200\" height=\"40\"></canvas></div>"
        "  <div class=\"sparkline-item\"><h4>Battery</h4><canvas id=\"spark-batt\" width=\"200\" height=\"40\"></canvas></div>"
        "</div>"
        "<div id=\"preview-box\">"
        ...
        "    if (range < 0.1) { range = 0.2; minVal -= 0.1; }"
        "    ctx.font = '10px monospace';"
        "    ctx.fillStyle = '#888';"
        "    ctx.textAlign = 'right';"
        "    ctx.fillText(maxVal.toFixed(1) + unit, leftMargin - 5, 10);"
        "    ctx.fillText(minVal.toFixed(1) + unit, leftMargin - 5, height - 2);"
        ...
        "    };"
        "    const timestamps = rows.map(row => row.cells[1].textContent.split(' ')[1]);"
        "    document.getElementById('sparkline-timestamp-range').textContent = `Time range: ${timestamps[0]} - ${timestamps[timestamps.length-1]}`;"
        "    const tempData = getColumnData('temperature_c');"
        "    const pressData = getColumnData('pressure_pa');"
        ...
        "    drawSparkline('spark-press', pressData, '#bb86fc', 'Pa');"
        "    drawSparkline('spark-diff-press', diffPressData, '#cf6679', 'Pa');"
        "    drawSparkline('spark-batt', battData, '#03dac6', 'V', 3.0, 4.2);"
        "}"
        "updateSensorData();"
